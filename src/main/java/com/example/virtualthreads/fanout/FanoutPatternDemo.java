package com.example.virtualthreads.fanout;
 ...
 *
 * The fanout pattern involves concurrently performing multiple tasks (e.g., making
...
public final class FanoutPatternDemo {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        log.info(String.format("Processing %d requests, each fanning out to %d services (%d total concurrent operations)",
        ...
        runScenario("Platform Thread Pool",
    ...
    private static void runScenario(String label,
        ...
        try {
            var tasks = IntStream.range(0, NUM_REQUESTS)
        ...
        log.info(String.format("[%s] Total time: %d ms (%.2f requests/sec, %d total operations)",
                label, totalTime.toMillis(), throughput, NUM_REQUESTS * FANOUT_SIZE));
    }

    /**
     * Handles a request by fanning out to multiple services concurrently.
     * Uses the shared fanout executor for parallel service calls.
     */
    private static void handleRequest(String userId, String scenario, ExecutorService fanoutExecutor) {
        Instant start = Instant.now();
        ...
        try {
            ...
            // Aggregate results
            String aggregatedResponse = aggregateResponse(userProfile, orders, recommendations, notifications);

            Duration elapsed = Duration.between(start, Instant.now());
            if (userId.equals("User0") || userId.equals("User99")) {
                log.info(String.format("[%s] Request for %s completed in %d ms on %s",
    ...
    private static String fetchRecommendations(String userId) throws InterruptedException {
        log.info(String.format("Fetching recommendations for %s on %s", userId, Thread.currentThread()));
     ...
     */
    private static String aggregateResponse(String profile, String orders,
                                           String recommendations, String notifications) {
        return String.format("{%s, %s, %s, %s}", profile, orders, recommendations, notifications);
    }

    /**
     * Example with real HTTP calls (commented out to avoid external dependencies).
     * Uncomment and modify URLs to test with actual HTTP endpoints.
     */
