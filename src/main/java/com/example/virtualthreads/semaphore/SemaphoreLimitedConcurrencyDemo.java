package com.example.virtualthreads.semaphore;
 ...
 */
public final class SemaphoreLimitedConcurrencyDemo {
    ...
    public static void main(String[] args) {
            ...
            for (int task = 0; task < TOTAL_TASKS; task++) {
                ...
                executor.submit(() -> {
                    ...
                    try {
                        ...
                        System.out.printf(
                                "Task %d running on %s (in-flight=%d)%n",
                                taskId,
                                thread,
                                current);
                        simulateWork();
        ...
        System.out.printf(
                "Completed %,d tasks with max %,d concurrent virtual threads in %d ms (peak observed %,d).%n",
                TOTAL_TASKS,
                MAX_CONCURRENCY,
                elapsed.toMillis(),
                peakConcurrency.get());
